...
| `reader_bench` | Request-body reader cost of client-streaming calls, without a network: envelope-framed messages go through `ConnectRpcService` from an in-memory body (ready, chunked, and pending before every frame). Run it on a dedicated machine; it has no client or server to contend with. | `cargo bench --bench reader_bench` |
